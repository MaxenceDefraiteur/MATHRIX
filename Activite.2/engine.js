...
function commande(cmd) {
	if (cmd == 'infecter') {
		...
		afficherImage('intro.png');
		 ...
		 cacherImage();
		 textePersistant("Jour 0 :\nConvertir un nombre décimal\nen nombre binaire",0,1);
		//couleurAttenuer();
		afficherImage('jour0.png');
	}  else if (cmd == 'jour1') {
		 cacherImage();
		 textePersistant("Jour 1 : Encodage des lettres",0,1);
		//couleurAttenuer();
		afficherImage('jour1.png');
	} else if (cmd == 'jour2') {
		cacherImage();
		 textePersistant("Jour 2 : Opérateur logique XOR",0,1);
		//couleurAttenuer();
		afficherImage('jour2.png');
	} else if (cmd == 'jour3') {
		cacherImage();
		 textePersistant("Jour 3 : Principe du\nchiffrement    ",0,1);
		//couleurAttenuer();
		...
		cacherImage();
		 textePersistant("Jour 4 : Implémenter le\nchiffrement sur\n Scratch   ",0,1);
		//couleurAttenuer();
		afficherImage('jour4.png');
		afficherLien();
	} else if (cmd == 'mission finale') {
		cacherImage();
		cacherLien();
		 textePersistant("Mission Finale : Décrypter\nle message envoyé\npar le capitaine",0,1);
		//couleurAttenuer();
		//afficherLien();
		afficherImage('missionfinale.png');
	} else if (cmd == 'jour bonus') {
		cacherImage();
		 textePersistant("Jour Bonus : Programme Scratch\n:convertir un nombre décimal\nen nombre binaire",0,1);
		//couleurAttenuer();
		//afficherLien();
		afficherImage('jourbonus.png');
	} else if (cmd == 'lien') {
